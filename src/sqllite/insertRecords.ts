...
const insertRecords = async (dbReturn: any, downStreamresponse: Forward) => {
  ...
  if (downStreamresponse.external_failure == "TEMPORARY_CHANNEL_FAILURE") {
    await tempFailuresTable.run(
      ...
      downStreamresponse.out_pubkey,
      downStreamresponse.in_channel_alias,
      downStreamresponse.out_channel_alias,
      downStreamresponse.tokens,
      downStreamresponse.fee,
      downStreamresponse.external_failure
    );
