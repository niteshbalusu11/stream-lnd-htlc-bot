...
const readDB = async () => {
  ...
  const in_temp_channel_failures =
    "SELECT *, count(in_pubkey) FROM temp_channel_failures group by in_pubkey";

  const in_downstream_failures =
    "SELECT *, count(in_pubkey) FROM downstream_failures group by in_pubkey";

  const out_temp_channel_failures =
    "SELECT *, count(out_pubkey) FROM temp_channel_failures group by out_pubkey";

  const out_downstream_failures =
    "SELECT *, count(out_pubkey) FROM downstream_failures group by out_pubkey";
