...
const createTables = async (db) => {
    ...
    const tempFailuresTable = await db.prepare("INSERT INTO temp_channel_failures (at, in_channel, out_channel, in_pubkey, out_pubkey, in_channel_alias, out_channel_alias, tokens, fee, failure) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    const downFailuresTable = await db.prepare("INSERT INTO downstream_failures (at, in_channel, out_channel, in_pubkey, out_pubkey, in_channel_alias, out_channel_alias, tokens, fee, failure) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    return [tempFailuresTable, downFailuresTable];
};
await createDB();
